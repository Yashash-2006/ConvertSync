...
import { useProtectPdf, useDeletePdfOperation } from "@workspace/api-client-react";
import { Lock, Loader2, CheckCircle2, AlertCircle, Download, Trash2, Eye, EyeOff } from "lucide-react";
...
export default function Protect() {
  ...
  const [status, setStatus] = useState<"idle" | "protecting" | "done" | "error">("idle");
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const protectPdf = useProtectPdf();
  ...
  const handleProtect = async () => {
    ...
    try {
      ...
      setStatus("error");
      setErrorMsg(err instanceof Error ? err.message : "Protection failed");
    }
  };
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = resultFilename ?? "protected.pdf";
    a.click();
  };
  ...
  return (
    <div className="container mx-auto max-w-3xl px-4 py-10">
      <div className="mb-8 text-center">
        ...
        <p className="text-muted-foreground mt-2">Password-protect your PDF with 128-bit AES encryption.</p>
      </div>
      ...
      {!file && (
        <div
          className={cn(
            ...
            isDragging ? "border-purple-500 bg-purple-500/5" : "border-muted-foreground/30 hover:border-purple-500/60",
          )}
          ...
          <Card className="p-6 mb-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                The password will be required to open the protected PDF.
              </p>
            </div>
          </Card>
      ...
      {status === "protecting" && (
        <Card className="p-8 flex flex-col items-center gap-4">
          ...
          <p className="font-medium">Encrypting PDF…</p>
        </Card>
      )}
      ...
      {status === "done" && (
        <Card className="p-8 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <p className="font-semibold text-lg">PDF protected!</p>
            <p className="text-sm text-muted-foreground mt-1">Secured with 128-bit AES encryption.</p>
          </div>
      ...
      {status === "error" && (
        <Card className="p-8 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <p className="font-semibold text-lg">Protection failed</p>
