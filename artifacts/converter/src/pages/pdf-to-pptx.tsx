...
import { usePdfToPptx, useDeletePdfOperation } from "@workspace/api-client-react";
...
export default function PdfToPptx() {
  ...
  const handleRun = async () => {
    ...
    try {
      const result = await pdfToPptx.mutateAsync({ data: { file } });
      setResultId(result.id); setResultFilename(result.resultFilename ?? "presentation.pptx"); setStatus("done");
    } catch (err) { setStatus("error"); setErrorMsg(err instanceof Error ? err.message : "Conversion failed"); }
  };
  const handleDownload = () => {
    if (!resultId) return;
    const a = document.createElement("a"); a.href = `${import.meta.env.BASE_URL}api/pdf/${resultId}/download`; a.download = resultFilename ?? "presentation.pptx"; a.click();
  };
  ...
  return (
    <div className="container mx-auto max-w-3xl px-4 py-10">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-3xl font-bold tracking-tight">PDF → PowerPoint</h1>
        <p className="text-muted-foreground mt-2">Convert a PDF into an editable PPTX presentation using LibreOffice.</p>
      </div>
      ...
      {file && status === "idle" && (
        <>
          ...
          <Card className="p-5 mb-6 bg-muted/40 text-sm text-muted-foreground">
            Conversion quality depends on PDF content. Text-based PDFs convert better than scanned documents. Consider running OCR first on scanned files.
          </Card>
          <Button className="w-full bg-orange-500 hover:bg-orange-600" size="lg" onClick={handleRun}>
            <Presentation className="w-4 h-4 mr-2" />Convert to PowerPoint
          </Button>
        </>
      )}

      {status === "processing" && (<Card className="p-8 flex flex-col items-center gap-4 text-center"><Loader2 className="w-10 h-10 animate-spin text-orange-500" /><div><p className="font-medium">Converting…</p><p className="text-sm text-muted-foreground mt-1">This can take 30–60 seconds.</p></div></Card>)}
      {status === "done" && (
        <Card className="p-8 flex flex-col items-center gap-4 text-center">
          <CheckCircle2 className="w-12 h-12 text-green-500" /><p className="font-semibold text-lg">PowerPoint ready!</p>
          <div className="flex gap-3">
            <Button className="bg-orange-500 hover:bg-orange-600" onClick={handleDownload}><Download className="w-4 h-4 mr-2" />Download PPTX</Button>
